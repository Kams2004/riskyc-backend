...
# Works whether this script sits at ~/riskyc/riskyc.sh (siblings alongside it)
# or was left inside ~/riskyc/riskyc-backend/riskyc.sh (one level up).
...
# Some hosts only have the standalone `docker-compose` (v1) binary, others
# only the `docker compose` (v2) plugin — support either transparently.
if docker compose version >/dev/null 2>&1; then
  DC=(docker compose)
elif command -v docker-compose >/dev/null 2>&1; then
  DC=(docker-compose)
else
  err "Neither 'docker compose' (plugin) nor 'docker-compose' (standalone) was found."
...
compose_backend()  { (cd "$BACKEND_DIR"  && "${DC[@]}" "$@"); }
compose_frontend() { (cd "$FRONTEND_DIR" && "${DC[@]}" "$@"); }
